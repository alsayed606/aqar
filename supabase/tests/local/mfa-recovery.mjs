// Recovery codes and the e-mail fallback (migration 0071).
//
...
// A caller is an identity, a session, AND an assurance level — 0071 reads the `aal` claim to know
...
try {
  ...
  const ledger = await one("select verified from app.schema_migration where version='0071'");
  ok("0071 recorded itself in the ledger", ledger !== undefined, JSON.stringify(ledger));
} finally {
  await stop();
}
