// build-schema.mjs — regenerates schema_all.sql from migrations/*.sql (Charter هـ-4, مر-18).
// SOURCE OF TRUTH = supabase/migrations/. Run:  node supabase/build-schema.mjs  (or npm run build:schema)
import { readFileSync, readdirSync, writeFileSync } from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";

const HERE = path.dirname(fileURLToPath(import.meta.url));
const MIG = path.join(HERE, "migrations");
const OUT = path.join(HERE, "schema_all.sql");
const BAR = "-- " + "=".repeat(64);

const header =
  "-- schema_all.sql — GENERATED from supabase/migrations/*.sql by supabase/build-schema.mjs.\n" +
  "-- SOURCE OF TRUTH = migrations/ (Charter هـ-4). DO NOT EDIT BY HAND; run:\n" +
  "--   node supabase/build-schema.mjs   (or: npm run build:schema)\n" +
  "-- Convenience for one-shot apply (e.g. Supabase SQL Editor). Verified: loads clean on PostgreSQL 17.\n";

const files = readdirSync(MIG).filter((f) => f.endsWith(".sql")).sort();
let out = header;
for (const f of files) {
  // Normalize CRLF -> LF so output is byte-identical on Windows and Linux (CI drift check).
  const body = readFileSync(path.join(MIG, f), "utf8").replace(/\r\n/g, "\n").replace(/\s*$/, "");
  out += `\n${BAR}\n-- ${f}\n${BAR}\n${body}\n`;
}
writeFileSync(OUT, out, "utf8");
console.log(`schema_all.sql regenerated from ${files.length} migrations (${files[0]} … ${files[files.length - 1]}).`);

// The list the RUNNING CODE expects. The platform health page compares it against
// app.schema_migration — what the database says it actually has — and the difference is the answer
// to "is this deployment complete?". Generated here so the two can never be edited apart: the
// migrations directory is the single source, exactly as it is for schema_all.sql.
const LIST = path.resolve(HERE, "..", "lib", "migrations.ts");
const entries = files.map((f) => `  { version: "${f.slice(0, 4)}", name: "${f.replace(/\.sql$/, "")}" },`);
writeFileSync(
  LIST,
  `// GENERATED by supabase/build-schema.mjs from supabase/migrations/*.sql — DO NOT EDIT BY HAND.\n` +
    `// Run: node supabase/build-schema.mjs   (or: npm run build:schema)\n\n` +
    `export type ExpectedMigration = { version: string; name: string };\n\n` +
    `/** Every migration this build of the app expects the database to have. */\n` +
    `export const EXPECTED_MIGRATIONS: ExpectedMigration[] = [\n${entries.join("\n")}\n];\n`,
  "utf8",
);
console.log(`lib/migrations.ts regenerated with ${files.length} expected migrations.`);
