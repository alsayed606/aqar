-- schema_all.sql — the migrations in supabase/migrations/ concatenated in order for one-shot apply
-- (e.g. Supabase SQL Editor). SOURCE OF TRUTH = migrations/ (Charter هـ-4); this file is maintained
-- by appending each new migration under a "-- NNNN_name.sql" separator header. Automating this
-- concatenation is tracked for Sprint B. Safe to run on a fresh DB. Verified: loads clean on PostgreSQL 17.
