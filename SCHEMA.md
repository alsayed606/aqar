...
> ⚠️ **هذا الجدول يتوقّف عند `0045`** ولم يُحدَّث بعدها. المصدر الحيّ هو مجلّد `supabase/migrations/` نفسه، ولمعرفة **أيّها مطبَّق فعلاً** انظر البند التالي.

### سجلّ الهجرات | Migration ledger (`0068`)

**كل هجرة تُسجّل نفسها** — سطرها الأخير:

```sql
select app.record_migration('0069', '0069_your_migration');
```

| | |
|---|---|
| `app.schema_migration` | ما تملكه القاعدة فعلاً. بلا سياسة RLS: لا يُقرأ عبر PostgREST |
| `app.migration_status()` | قراءته — **لمشغّلي المنصة فقط**، لأن قائمة الناقص خريطة للحرّاس الغائبين |
| `lib/migrations.ts` | ما يتوقّعه **الكود** — مولّد من مجلّد الهجرات بـ`npm run build:schema` |
| `/platform/health` | يقارن الاثنين ويعرض **الفرق** |

**ولماذا وُجد:** ٦٧ هجرة طُبّقت يدوياً، وسجلّها الوحيد قائمة في `CHANGELOG.md` تُحدَّث من الذاكرة. قالت إن `0029` مطبَّقة ولم تكن — **فبقيت بوابة المستأجر معطّلة شهوراً**، لأن استدعاء اللوحة الرئيسية (`my_tenant_links`) يبتلع خطأه، فبدت الهجرة الغائبة كمستأجر بلا بوابة.

**والبذر لـ`0001`–`0067` استُنتج من المخطّط نفسه** لا من قائمة: لكل هجرة **مِجَسّ** يبحث عن الأثر الذي تركته. وأربع هجرات (`0014`, `0046`, `0055`, `0056`) **تُعيد إصدار دوالّ قائمة ولا تُنشئ شيئاً جديداً**، فلا مِجَسّ يميّزها — وتُسجَّل `verified = false` بدل ادّعاء التحقّق.
...
- **صفحة المكتب والأذرع (`0050`):** `platform_tenant_360(org)` (jsonb: المكتب · الاشتراك · الاستهلاك والحدود · **المحفظة أعداداً** · ما دفعه لنا · البطاقة · **فريق المكتب** · النشاط) · `operator_extend_trial(org, days, reason?)` (تحسب من **اليوم** لا من نهاية تجربة انقضت؛ تكتب تدقيقاً؛ ترفض غير التجريبي) · `platform_identity_activity()` (الموضع **الوحيد** الذي يعرف أن أوقات الدخول في `auth.users`؛ `platform_org_activity` صارت تجميعاً فوقها).
- **حالة `suspended`** أُضيفت لـ`subscription_status`: «أوقفناه» ≠ «غادر» — وخلطهما يُفسد حساب الفقد. `subscription_active()` تفشل مغلقةً فتمنع الإنشاء الجديد تلقائياً وتبقي البيانات القائمة قابلة للتعديل.
- **الاشتراكات والفوترة (`0051`):** `operator_upsert_plan(...)` (تحرير الكتالوج من اللوحة — الأسعار والحدود **بيانات** منذ `0036`، وهذه أول طريق محروس ومُدقَّق لتغييرها؛ التدقيق بـ`org_id = null` لأنها عملية منصة) · `platform_list_payments(search?, status?, limit, offset)` · `platform_billing_health(days)` (نسبة النجاح **`null` لا ١٠٠٪ حين لا محاولات**؛ أسباب الفشل تُقرأ من `raw` بسلسلة مواضع معتادة وتُترك بلا اسم إن لم تُذكر) · `platform_subscription_center()` (التجارب والتجديدات و**«نشط بلا بطاقة محفوظة»**).
- **غير مبني بقرار:** الخصومات/الكوبونات (لا نموذج) · **تنفيذ** الاسترداد (يحتاج واجهة البوابة) · **فاتورة ضريبية للمكتب عن رسوم الاشتراك (فجوة ZATCA على المنصة نفسها)** · ميزات لكل خطة (مؤجَّلة لـT‑5).
- **الصحة والتنبيهات والتدقيق (`0052`):** **`app.cron_run`** (سجلّ كل دورة مجدولة — يكتبه `record_cron_run` من مسارَي الـcron بصلاحية service_role حصراً؛ RLS بلا policy) · `platform_health()` · `platform_alerts()` (تُعيد `kind` ثابتاً و**لا تُعيد ما عدده صفر**؛ الصياغة والوجهة في الواجهة فـSQL بلا مسارات) · `platform_list_audit(...)` + `platform_audit_actions()`.
- **خط العزل في التدقيق:** البيانات الوصفية للعملية (من·ماذا·لأي مكتب·متى) ليست بيانات مكتب — هي شكل النشاط، من فئة العدد. أما **حمولة `detail` فتُعرض لعمليات `platform.*` فقط**؛ حمولة عمليات المكاتب **محجوبة**.
- 🔴 **`0053` إصلاح أمني:** `alter default privileges` في `0001` يمنح `anon`/`authenticated` تنفيذ **كل** دالة جديدة، فـ`revoke from public` وحده **لا يغلق** دوال service_role. **قاعدة ملزمة: دالة بلا فحص تصريح داخلي يجب أن تسحب المنح من `anon` و`authenticated` صراحةً.** (المحروسة غير متأثرة — فحصها داخلي.)
- **الإعدادات والميزات والبثّ (`0054`):** **`app.platform_setting`** (مدة التجربة · خطة البداية · بريد/جوال الدعم · اسم مُرسِل البثّ — و`create_organization` صارت تقرأ الأولين بدل ثابتين في الكود) · **`app.platform_flag`** طبقة عامة فوق `feature_flag` لكل مكتب، و`feature_enabled(org,key)` تحسم بترتيب: **استثناء المكتب ← الخطة المطلوبة ← مفعّلة للجميع ← نسبة الإطلاق**، و**المجهول مغلق**، والنسبة تُجزَّأ من (المكتب+المفتاح) فتثبت للمكتب ولا تتقاطع بين ميزتين · **`app.broadcast`** + `platform_broadcast(..., p_dry_run)` حيث **المعاينة تحسب ولا تكتب**، والإرسال يعيد استخدام طابور الإشعارات والمُصرِّف المجدول.
- **مفاتيح المزوّدين ليست إعدادات:** تبقى متغيّرات بيئة؛ صفحة الإعدادات تعرض **حالتها فقط** ولا تقرأ قيمتها — لوحةٌ تقرأها تحوّل اختراق حساب مشغّل إلى اختراق حساب الدفع.
- **السطح:** `app/platform/` (layout يحمل بوّابة `is_platform_operator()` + قائمة جانبية مستقلة). `/operator` حُذف في T‑1. الوحدات: اللوحة التنفيذية · المكاتب + Tenant 360 · الاشتراكات · الفوترة · الصحة · التنبيهات · التدقيق · البثّ · الميزات · الإعدادات. **مؤجَّل بقرار:** الدعم · تحليلات المنتج.
