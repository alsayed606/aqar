...
export function OwnerForm() {
  ...
  return (
    <form ref={formRef} action={action} className="grid gap-3 sm:grid-cols-2">
      ...
      <div>
        <label className="mb-1 block text-sm font-medium" htmlFor="legal_kind">النوع</label>
        <select
          id="legal_kind"
          name="legal_kind"
      ...
      <div>
        <label className="mb-1 block text-sm font-medium" htmlFor="national_id">رقم الهوية / السجل</label>
      ...
      <div>
        <label className="mb-1 block text-sm font-medium" htmlFor="phone">الجوال</label>
        <input id="phone" name="phone" dir="ltr" placeholder="05XXXXXXXX"
          className="w-full rounded-lg border border-neutral-300 bg-transparent px-3 py-2 text-right outline-none focus:border-brand dark:border-neutral-700" />
      </div>

      <div>
        <label className="mb-1 block text-sm font-medium" htmlFor="iban">الآيبان (IBAN)</label>
        <input id="iban" name="iban" dir="ltr" placeholder="SA.."
          className="w-full rounded-lg border border-neutral-300 bg-transparent px-3 py-2 text-right outline-none focus:border-brand dark:border-neutral-700" />
      </div>

      <div>
        <label className="mb-1 block text-sm font-medium" htmlFor="bank_name">البنك</label>
      ...
      <div className="sm:col-span-2">
        <button type="submit" disabled={pending}
          ...
          {pending ? "جارٍ الحفظ…" : "إضافة المالك"}
        </button>
      </div>
    </form>
  );
